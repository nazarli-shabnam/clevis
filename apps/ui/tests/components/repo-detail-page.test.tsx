...
describe("RepoDetailPage", () => {
  ...
  it("does not auto-resolve a token when the Actions Cache tab is opened", async () => {
    renderPage();
    await waitFor(() => expect(tokensResolveMock).toHaveBeenCalledTimes(1));

    fireEvent.click(screen.getByRole("tab", { name: /actions cache/i }));

    await waitFor(() => expect(screen.getByRole("button", { name: /load caches/i })).toBeInTheDocument());
  ...
  it("loads caches via the GitHub App path without a PAT field on the Actions Cache tab", async () => {
    cacheListMock.mockResolvedValue({ repository: "acme/demo", total: 0, actions_caches: [] });
    ...
    await waitFor(() => expect(cacheListMock).toHaveBeenCalledWith("acme", "demo"));
  });
  ...
  it("resets armed clear state when navigating to a different repo under the same org", async () => {
