...
def sync_org_admin_memberships(db: Session, user: User, user_token: str) -> None:
    ...
    for gh_membership in memberships:
        ...
        if existing is not None and existing.role == "admin":
            continue
        # The admin role now comes from GitHub, so the promotion also marks the row
        # source='github' in the same write: an invite-sourced row promoted here would otherwise
        # survive GitHub removal as an admin. (Fail-closed: losing GitHub admin then revokes the
        # row; re-invite if needed.)
        org_membership_repo.get_or_create(db, org_id=org.id, user_id=user.id, role="admin", source="github")
